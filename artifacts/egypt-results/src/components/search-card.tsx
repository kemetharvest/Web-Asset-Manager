  ...
  return (
    <section id="search-section" className="w-full max-w-lg mx-auto">
      ...
      >
        ...
        {/* ── Tab content
            Both tabs are always in the DOM (no AnimatePresence / DOM removal).
            Visibility is toggled with CSS only — avoids insertBefore/removeChild
            errors from React 19 + Framer Motion AnimatePresence interactions.
        ── */}
        <div className="p-4">

          {/* ── Seat tab ── */}
          <div className={tab === 'seat' ? 'block' : 'hidden'}>
            <form
            ...
            >
              ...
              {/* error — CSS opacity transition, no AnimatePresence */}
              <div
                className={cn(
                  'transition-all duration-200 overflow-hidden',
                  seatError ? 'opacity-100 max-h-10' : 'opacity-0 max-h-0'
                )}
              >
                <p className="flex items-center gap-1.5 text-destructive text-xs font-medium">
            ...
            {/* Recent searches — CSS transition, no AnimatePresence */}
            <div
              className={cn(
                'transition-all duration-300 overflow-hidden',
                recentSeats.length > 0 ? 'opacity-100 max-h-40 mt-3' : 'opacity-0 max-h-0'
              )}
            >
          ...
          {/* ── Name tab ── */}
          <div className={tab === 'name' ? 'block' : 'hidden'}>
            <div className="space-y-3">
              ...
              {/* error — CSS only */}
              {nameError && (
                <p className="text-center py-4 text-destructive text-xs">{nameError}</p>
              )}
              ...
              {nameResults && nameResults.students.length > 0 && (
                <div className="space-y-3">
                  ...
                  <div className="space-y-1.5 max-h-72 overflow-y-auto">
                    {nameResults.students.map((student) => (
                      <button
